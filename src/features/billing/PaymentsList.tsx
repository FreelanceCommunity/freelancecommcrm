...
export default function PaymentsComponent() {
  const { data, isLoading } = useQuery({
    queryKey: ['payments', 'Payments'],
    queryFn: async () => {
      try {
        const { data, error } = await supabase.from('payments').select('*').limit(20);
        if (error) return [];
        return data || [];
      } catch {
        return [];
  ...
  if (isLoading) return <div className="p-10">Loading Payments...</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Payments Records</CardTitle>
        </CardHeader>
        <CardContent>
          {!data || data.length === 0 ? (
