...
export default function ActivitiesComponent() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        const { data, error } = await supabase.from('activities').select('*').limit(20);
        if (error) return [];
        return data || [];
      } catch (err) {
        return [];
  ...
  if (isLoading) return <div className="p-10">Loading Activities...</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Activities Records</CardTitle>
        </CardHeader>
        <CardContent>
          {!data || data.length === 0 ? (
