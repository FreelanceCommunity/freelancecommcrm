...
export default function MyCompanyComponent() {
  const { data, isLoading } = useQuery({
    queryKey: ['clients', 'My Company'],
    queryFn: async () => {
      try {
  ...
  if (isLoading) return <div className="p-10">Loading My Company...</div>;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>My Company Records</CardTitle>
        </CardHeader>
        <CardContent>
          {!data || data.length === 0 ? (
